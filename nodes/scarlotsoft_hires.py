...
class ScarlotSoft_UpscalerHiResFix:
    ...
    def process(self, model, positive, negative, image, vae, upscale_model, upscale_method, scale_by, seed, steps, cfg, sampler_name, scheduler, denoise, **kwargs):
        ...
        # 4. VAE Encode (TILED con Overlap para evitar cuadrículas)
        encoder = nodes.VAEEncodeTiled()
        latents = encoder.encode(vae, scaled_image, tile_size=512, overlap=64)[0]
        ...
        # 6. VAE Decode (TILED con Overlap para evitar cuadrículas)
        decoder = nodes.VAEDecodeTiled()
        final_image = decoder.decode(vae, sampled_latents, tile_size=512, overlap=64)[0]
